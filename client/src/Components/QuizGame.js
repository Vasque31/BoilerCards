...
function QuizGame() {
  ...
  const [cardsQuiz, setCardsQuiz] = useState(
    Object.values(flashcardsetinfo.flashcardarray)
  );
  ...
  if (readyForNewQuestion) {
    if (mode == "All prompts once") {
      selectedPromptIndex = randomCard(
        previousCorrectPrompts.concat(previousIncorrectPrompts)
      );
      ...
      if (
        previousCorrectPrompts.concat(previousIncorrectPrompts).length >=
        cardsQuiz.length
      ) {
        handleShowExitQuiz();
      }
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (i != 3)
        answerIndices.push({
          Index: selectedIncorrectAnswers[i],
          Correct: false,
        }); //add with index of incorrect array, not out of bounds
    }
    ...
    <div>
      ...
      <Modal show={showContinueorExit} onHide={() => handleHideExit}>
        ...
        <Modal.Body>
          ...
          <br></br>
          <h1>
            Time: {(time - (time % 100)) / 100}.{(time % 100) / 10}sec
          </h1>
        </Modal.Body>
        <Modal.Footer>
