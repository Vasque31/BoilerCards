export class userDBService {
  ...
  async AddAsync(client,newListing){
    const result = await client.db("User").collection("Userdata").insertOne(newListing);
    console.log(`New listing created with the following id: ${result.insertedId}`);
}
  async AddLogAsync(client,newListing){
  const result = await client.db("User").collection("Userlog").insertOne(newListing);
  console.log(`New listing created with the following id: ${result.insertedId}`);
  }
...
async  GetAsync(client, nameOfListing){
  const result = await client.db("User").collection("Userdata").findOne({ username: nameOfListing });
